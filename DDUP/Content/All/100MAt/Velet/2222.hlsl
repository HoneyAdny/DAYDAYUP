...
void main()
{
    ...
    vec3 _93 = _44._m6[2u].xyz;//(1,0.911,0.911)
    vec2 _100 = _44._m6[3u].xy;//(-0.2,1.2)
    float _106 = _44._m6[4u].x;//0.35
    float _111 = _44._m6[5u].x;//0.4
    float _114 = _44._m6[6u].x;//2
    vec2 _117 = _44._m6[7u].xy;//(1,0.5)
    vec3 _122 = _44._m6[1u].xyz;//（ 1,1,1）
    float _126 = _44._m6[0u].x;//0
    float _129 = _44._m0[3u].x;//6
    vec3 _132 = _44._m0[2u].xyz;//(0.58,0.51,0.51)
    float _136 = _44._m0[0u].x;//感觉能控制亮度//   1
    vec4 _140 = _9;
    ...
    vec3 _933Other = normalize((_164法线基向量.xyz * _930.zzz) + ((_168副切线 * _930.yyy) + (_159切线 * _930.xxx)));
    vec3 _950FinalOtherVer = normalize(_933Other + (-(vec3(dot(_933Other, _332最终法线WS)) * _332最终法线WS)));//
    vec3 _961 = normalize(cross(_332最终法线WS, _950FinalOtherVer));
    ...
    float _1003 = dot(_950FinalOtherVer, _909HDir);
    ...
    float _1023 = dot(_950FinalOtherVer, _44._m3[2u].xyz);//(0.88,0.214,0.423)
    float _1029 = dot(_961, _44._m3[2u].xyz);//(0.88,0.214,0.423)
    float _1035 = _418花纹RGB2 * 0.9900000095367431640625;
    float _1039 = _459花纹RGB * _459花纹RGB;//a*a
    float _1043 = clamp(_1039 * (1.0 + _1035), 0.00999999977648258209228515625, 1.0);//花纹 黑纹白底
    float _1050 = clamp(_1039 * (1.0 + (-_1035)), 0.00999999977648258209228515625, 1.0);//花纹 
    vec3 _1057;
    _1057.x = _1043 * dot(_950FinalOtherVer, _217归一负摄像机);
    ...
    float _1110 = _1007;
    float _1007 = _1110;
    float _1113 = (((_1003 * _1003) / (_1043 * _1043)) + ((_1007 * _1007) / (_1050 * _1050))) + (_999 * _999);
    float _1134 = step(0.0, _999) / (((_1043 * _1050) * _1113) * _1113);
    ...
    float _1170 = (_44._m5[4u].z * 0.017453290522098541259765625) * clamp((_1133 * _1133) * 0.25, 0.0, 1.0);//0
    float _1182 = cos(_1170);//1
    vec3 _158a = ((vec3(_1182) * _158) + (vec3(sin(_1170)) * cross(vec3(0.57735002040863037109375), _158))) + ((vec3(dot(vec3(0.57735002040863037109375), _158)) * vec3(0.57735002040863037109375)) * vec3(1.0 + (-_1182)));
        //_158
    float _1210 = 1.0 + (-_966);
    float _1214 = _1210 * _1210;
    float _1209 = (_1214 * _1214) * _1210;
    vec3 _1223 = (vec3(clamp(50.0 * _158a.y, 0.0, 1.0) * _1209) + (vec3(1.0 + (-_1209)) * _158a)) * vec3(_1133);
    vec3 _1244 = _44._m1[2u].xyz;//-1.77
    vec3 _1248 = _44._m1[1u].xyz;
    vec3 _1252 = _44._m1[0u].xyz;
    vec3 _1242a = (_1244 * _164法线基向量.zzz) + ((_1248 * _164法线基向量.yyy) + (_1252 * _164法线基向量.xxx));
    vec3 _1242 = _1242a;
    float _1271 = length(_1242a.xy);
    vec2 _1280 = _1242a.xy / vec2(_1271 + 0.001000000047497451305389404296875);
    ...
    vec3 _1302SH = _1283.xyz * vec3(_1283.w * 16.0);//
    float _1311 = _44._m7[0u].x;//1.77
    vec3 _1378 = min(vec3(200.0), (((_901 + ((vec3(_155 * _916NoL) * _44._m3[0u].xyz) * ((_463暗Color + _974) + _1223))) + (vec3(((smoothstep(_100.x, _100.y, (dot(_117, _1242.xy) + (-_111)) * _106) * _44._m5[3u].w) * pow(1.0 + (-clamp(_970, 0.0, 1.0)), _114)) * _126) * _122)) + ((((_1302SH * _1302SH) * vec3(_1311)) * vec3(_156)) * _158)) + mix(vec3(0.0), vec3(0.0), vec3(_155)));
    //                               深色           阴影    NoL      (0.89,0.84,0.9)                                                         -0.2       1.2       (1,0.5)               0.4     0.35            1
    vec4 _153;
