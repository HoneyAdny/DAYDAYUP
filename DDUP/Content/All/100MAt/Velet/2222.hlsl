...
void main()
{
    ...
    vec3 _281 = mix(_268.xyz, vec3(0.0, 0.0, 1.0), _44._m5[5u].yyy);
    ...
    vec3 _909 = normalize(_217归一负摄像机 + _44._m3[2u].xyz);//(0.88,0.214,0.423)
    float _916 = max(0.0, dot(_332最终法线WS, _44._m3[2u].xyz));//(0.88,0.214,0.423)
    vec3 _923;
    _923.z = 0.0;
    _923 = vec3(_44._m5[4u].xy.x, _44._m5[4u].xy.y, _923.z);
    vec3 _930 = normalize(_923);
    vec3 _933 = normalize((_164法线基向量.xyz * _930.zzz) + ((_168副切线 * _930.yyy) + (_159切线 * _930.xxx)));
    vec3 _950 = normalize(_933 + (-(vec3(dot(_933, _332最终法线WS)) * _332最终法线WS)));
    vec3 _961 = normalize(cross(_332最终法线WS, _950));
    float _966 = dot(_909, _217归一负摄像机);
    ...
    float _999 = dot(_332最终法线WS, _909);
    float _1003 = dot(_950, _909);
    float _1007 = dot(_961, _909);
    float _1011 = abs(dot(_332最终法线WS, _217归一负摄像机));
    float _1016 = abs(dot(_332最终法线WS, _44._m3[2u].xyz));
    float _1023 = dot(_950, _44._m3[2u].xyz);
    ...
    vec3 _1057;
    _1057.x = _1043 * dot(_950, _217归一负摄像机);
    ...
    vec3 _1378 = min(vec3(200.0), (((_901 + ((vec3(_155 * _916) * _44._m3[0u].xyz) * ((_463暗Color + _974) + _1223))) + (vec3(((smoothstep(_100.x, _100.y, (dot(_117, _1242.xy) + (-_111)) * _106) * _44._m5[3u].w) * pow(1.0 + (-clamp(_970, 0.0, 1.0)), _114)) * _126) * _122)) + ((((_1302 * _1302) * vec3(_1311)) * vec3(_156)) * _158)) + mix(vec3(0.0), vec3(0.0), vec3(_155)));
    vec4 _153;
